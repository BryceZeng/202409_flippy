...
prompt_a, prompt_b = rp.gather(
    example_prompts, "victorial_dress victorial_dress".split()
)
prompt_a, prompt_b = rp.gather(
    example_prompts, "pencil_giraffe_head pencil_penguin".split()
)
...
print(
    "Every %i iterations we display an image in the form [image_a, image_b], where"
    % DISPLAY_INTERVAL
)
...
try:
    for iter_num in range(NUM_ITER):
        ...
        for label, learnable_image, weight in rp.random_batch(
            list(zip(labels, learnable_images, weights)), batch_size=1
        ):
            pred = s.train_step(
                label.embedding,
...
save_run(
    "-".join([prompt_a, prompt_b])
)  # You can give it a good custom name if you want!
